...
function create_new_explosion(name, tile, conditions, entity)
    local explosion = util.copyTable(
        data.raw["explosion"]["nuke-effects-vulcanus"])
    explosion.name = name
    explosion.surface_conditions = conditions
    explosion.created_effect.action_delivery.target_effects = {
        type = "set-tile", tile_name = tile, radius = 1
    }
    data:extend({ explosion })
    table.insert(entity.dying_trigger_effect, {
        type = "create-entity", check_buildability = true, entity_name = name
    })
end
...
deeplm_item.icons = {
    { icon = "__base__/graphics/icons/land-mine.png" }, {
        icon = "__base__/graphics/icons/explosives.png", scale = 0.33,
        shift = { 8, -8 }
    }
}
-- entity
...
deeplm_entity.surface_conditions = { { property = "gravity", min = 1 } }
deeplm_entity.flags = {
    "placeable-player", "placeable-enemy", "player-creation", "not-on-map"
}
...
create_new_explosion("deep-mine-explosion-vulcanus", "lava",
    { { property = "pressure", min = 4000, max = 4000 } }, deeplm_entity)
create_new_explosion("deep-mine-explosion-nauvis", "water-shallow",
    { { property = "pressure", min = 1000, max = 2000 } }, deeplm_entity)
